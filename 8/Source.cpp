...
void runge_kutt_2(double* x, double* y, double* y1, size_t n){
	double h = (x[n - 1] - x[0]) / (n - 1);
	double k1, k2;					// для первого уравнения системы
	double k11, k21;				// для второго уравнения системы
	for (size_t i = 1; i < n; ++i)
	{
	    k1 = h*f1(y1[i - 1]);
...
void runge_kutt_4(double* x, double* y, double* y1, size_t n){
	...
	double h2 = h / 2.0;
	double h6 = h / 6.0;
	double k1, k2, k3, k4;					// для первого уравнения системы
	double k11, k21, k31, k41;				// для второго уравнения системы
	for (size_t i = 1; i < n; ++i)
	{
		k1 = f1(y1[i - 1]);
